...
# Самая длинная подстрока, не содержащая одинаковых символов подряд
'''with open ('1_0003.txt') as f:
    s = f. readline(). strip ()
k = m = 1
for i in range (1, len (s)):
    if s[il != s[i - 1]:
...
# Самая длинная подстрока, не содержащая определённых символов
'''with open ('9_0021.txt') as f:
    s = f.readline().strip()
k = m = 0
for i in range(len(s)):
...
'''with open ('9_0021.txt') as f:
    s = f.readline().strip()
s = 'yergfvuwcetxfccaserzdysrdxzctyserxzcxsrexdzcxrezcwrexczsexzsxcztwex'
s = s.replace('e',' ').replace('C', ' ').replace('f',' ').split()
print(max([len (x) for x in s]))'''

# Самая длинная подстрока, содержащая только определённые символы
'''with open ('11_0143.txt') as f:
    s = f.readline().strip()
k = m = 0
for i in range(len(s)) :
    if s[i] in '0123456789ABC':
        k += 1
        m = max (m, k)
    else:
        k = 0
...
# Самая длинная подстрока, не содержащая определённую подстроку
'''with open ('15_0001.txt) as f:
    s = f.readline().strip()
...
# Повторяющиеся шаблоны с перекрытиями (подг. задание) 
'''with open ('26_0001.txt' ) as f:
    s = f.readline().strip()
